...
# Processes a sample mutual inductance measurement: applies phase and leakage corrections,
# normalises MI to the normal-state value, and optionally converts normalised MI to
# penetration depth via an interpolation lookup table generated by CalcMutualInductance.
class MIData:
	...
	def phase_correct_data (self, X, Y, angle, Tmin=0, Tmax=None, T=None):
		"""
		Rotate the complex lock-in signal X+iY to remove cable/setup phase mixing.

		If angle is provided (e.g. taken from a MICalibration run), it is used directly.
		If angle is None, the correction angle is estimated from the mean signal in the
		temperature range [Tmin, Tmax], which should lie deep in the SC phase where MIx≈0.
		T must be provided when angle is None.
		"""
		if angle is None:
	...
	def convert_XY_to_MI (self, X, Y, f, I):
		# Lock-in output V = 2πf·I·MI; after phase correction Y→real(MI), X→imag(MI)
	...
	def median_filter (self, data, threshold=1, window=10000):
		"""
		Flag outliers relative to a rolling median.

		threshold is a *relative* value: a point is flagged if
		|data - median| / |median| > threshold.  The default of 1 therefore
		flags points that deviate by more than 100 % of the local median.
		Returns (bad_bool, good_bool) boolean masks of the same length as data.
		"""
	...
	def correct_leakage (self, MIx, MIy, T, leakage, residual_capacitance=None, n=1000):
		...
		# MIy at T=0 should be zero; any non-zero offset is residual capacitive coupling.
		# If not provided, estimate it as the minimum of the smoothed MIy curve.
	...
	def normalize_MI (self, MIx, MIy, T, Tmin=None, Tmax=None):
		# Normalise to the normal-state (high-T) MI so MIxnorm=1 above Tc and 0 deep in the SC phase
		if Tmax is None:
			Tmax = T[-1]
		if Tmin is None:
			Tmin = T[-100]
	...
	def convert_MI_to_PD (self, lookup_table_path, save_path=None):
		# Lookup table is generated by CalcMutualInductance.calc_MI_PD_array; columns are
		# [penetration_depth, MI, MI_norm]. Invert MI_norm(lam) → lam(MI_norm) by interpolation.
